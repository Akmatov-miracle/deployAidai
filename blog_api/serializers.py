...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        field = ('username', 'email', 'first_name', 'last_name', 'password', 'password2')

    @staticmethod
    def validate_first_name(self, value):
        if not value.istitle():
            raise serializers.ValidationError("Name must start with upper")
        return value
...
class PostSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        print("isn't work: ", images_data)
        # for image_data in images_data.getlist('images'):
        #     PostImages.objects.create(post=created_post, image=image_data)
        images_obj = [PostImages(post=created_post, image=image) for image in images_data.getlist('images')]
